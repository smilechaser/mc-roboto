...
class TestProtocolFactory(unittest.TestCase):
    ...
    def _check_packet(self, block_dig_packet):
        ...
        self.assertEqual(block_dig_packet.DIRECTION, protocol.Direction.TO_SERVER)
        ...
        for expected, actual in zip(EXPECTED_FIELDS, block_dig_packet.FIELDS):
            ...
            self.assertEqual(expected[0], actual[0],
                msg='Expected field name "{}" but got "{}".'.format(expected[0], actual[0])
            )
            self.assertEqual(expected[1], actual[1],
                msg='Expected field type "{}" but got "{}".'.format(expected[1], actual[1])
            )
